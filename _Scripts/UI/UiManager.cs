...
public class UiManager : MonoBehaviour, ITimeTracker
{
    ...
    public void ActiveIdeaPanel(bool active)
    {
        ...
        {
            if (ideaPanel.transform.GetChild(4).gameObject.activeSelf == true)
            {
                ideaPanel.transform.GetChild(4).gameObject.SetActive(false);
            }
    ...
    private void OnApplicationQuit()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Social" || (dataManager.userId == PhotonNetwork.CurrentRoom.Name))
        {
            ...
            if (dataManager.isSavedDataBeforeQuit < 25 || (largeMapData.saveCount < 43 && SceneManager.GetActiveScene().name == "Social"))
            {
    ...
    IEnumerator SaveDataU()
    {
         ...
         yield return new WaitUntil(() => ((dataManager.isSavedDataBeforeQuit >= 25 && largeMapData.saveCount >= 43 && SceneManager.GetActiveScene().name == "Social") ||
                                          (dataManager.isSavedDataBeforeQuit >= 25 && SceneManager.GetActiveScene().name != "Social")));
