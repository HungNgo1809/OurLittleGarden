...
public class QuestDailyTask : MonoBehaviour
{
    ...
    void Start()
    {
        behavior = GetComponent<QuestBehavior>();
    }
        ...
        if (taskList.Count > 0)
        {
            ...
            for (int i = 0; i < taskList.Count; i++)
            {
                ...
                if (TimeManager.Instance.timestamp.hour == taskList[i].time_.hour && TimeManager.Instance.timestamp.minute == taskList[i].time_.minute && !IsTaskDone(i)) // co the them 1 Hanh dong khac cho ngay dac biet - chua nghi nen lam gi
                {
                    ...
                    {
                        behavior.SwitchStateAction(QuestBehavior.NPCAction.DoNothing);
                    }
                    else if (taskList[i].TaskGoal == "MeetSophia")
                    {

                        behavior.SwitchStateAction(QuestBehavior.NPCAction.MeetSophia);
                    }
                    else if (taskList[i].TaskGoal == "GoingSomeWhere")
                    {

                        behavior.SwitchStateAction(QuestBehavior.NPCAction.GoingSomeWhere);
                    }
                    else if (taskList[i].TaskGoal == "Dancing")
                    {

                        behavior.SwitchStateAction(QuestBehavior.NPCAction.Dancing);
                    }
                    else if (taskList[i].TaskGoal == "Thinking")
                    {

                        behavior.SwitchStateAction(QuestBehavior.NPCAction.Thinking);
                    }
                    else if (taskList[i].TaskGoal == "MeetSeller")
                    {

                        behavior.SwitchStateAction(QuestBehavior.NPCAction.MeetSeller);
                    }
                    else if (taskList[i].TaskGoal == "MeetNPC")
                    {
                        behavior.NPCGonnaMeet = taskList[i].NPC;
                        behavior.SwitchStateAction(QuestBehavior.NPCAction.MeetNPC);
                    }
