...
{
    public static Shoper Instance { get; set; }
    ...
    public QuestDailyTask questTask;
    public FisherDailyTask fisherTask;
    public GenDailyTask[] genDailyTask;
    

    public DataManager dataManager;
    public PlayerControl playerControl;
    public Transform NPCDadStartPosiotion;
    public GameObject BeginConverstation;
    ...
    private void Start()
    {
        ...
        {
           StartCoroutine(NewbieStartConver());
    ...
    */
    ...
    public void ResetNPCDailyTask()
    {
        if (TimeManager.Instance.timestamp.hour >= 0 && TimeManager.Instance.timestamp.hour <= 5)
        {

            if (dailyTask.taskList.Count > 0)
            {

                dailyTask.taskList.Clear();
                dailyTask.isTasksCreated = false;
                dailyTask.currentTask = null;
            }
            if (sellerTask.taskList.Count > 0)
            {

                sellerTask.taskList.Clear();
                sellerTask.isTasksCreated = false;
                sellerTask.currentTask = null;
            }
            if (questTask.taskList.Count > 0)
            {

                questTask.taskList.Clear();
                questTask.isTasksCreated = false;
                questTask.currentTask = null;
            }
            if (fisherTask.taskList.Count > 0)
            {

                fisherTask.taskList.Clear();
                fisherTask.isTasksCreated = false;
                fisherTask.currentTask = null;
            }
            foreach(var child in genDailyTask)
            {
                if (child.taskList.Count > 0)
                {

                    child.taskList.Clear();
                    child.isTasksCreated = false;
    ...
    public IEnumerator NewbieStartConver()
    {
    
        yield return new WaitForSeconds(10);
        playerControl.NPCInteract_ = true;
        playerControl.animator.SetFloat("Speed",0);
        playerControl.animator.SetBool("Run", false);
        if (NPC1.activeSelf == false)
        {
            NPC1.SetActive(true);
            NPC1.gameObject.transform.position = NPCDadStartPosiotion.transform.position;
        }
        ...
        if (nav.isOnNavMesh)
        {
            ...
            nav.SetDestination(playerControl.transform.position);
        }
        
        while (Vector3.Distance(NPC1.gameObject.transform.position, playerControl.gameObject.transform.position) > 2.5)
        {
        
            nav.SetDestination(playerControl.transform.position);
            yield return null;
        }
        if(Vector3.Distance(NPC1.gameObject.transform.position, playerControl.gameObject.transform.position) <= 2.5)
        {
            nav.isStopped = true;
            nav.ResetPath();
            behavior.SwitchState(GenNPCBehavior.NPCState.Talking);
            BeginConverstation.gameObject.SetActive(true);
        }
    ...
    {
        StartCoroutine(CloseObject(obj));
    }


    public IEnumerator CloseObject(GameObject obj)
    {
        obj.SetActive(true);
    ...
    public void EndConver()
    {
       StartCoroutine(EndConverstation());
    }

    public IEnumerator EndConverstation()
    {
       
        playerControl.NPCInteract_ = false;
        StartCoroutine(turtorialStart());
        NavMeshAgent nav = NPC1.GetComponent<NavMeshAgent>();
        GenNPCBehavior behavior = NPC1.GetComponent<GenNPCBehavior>();
        if (nav.isOnNavMesh)
        {
            ...
            nav.SetDestination(NPCDadStartPosiotion.transform.position);
        }
        while (Vector3.Distance(NPC1.gameObject.transform.position, NPCDadStartPosiotion.transform.position) > 2.5)
        {
            nav.SetDestination(NPCDadStartPosiotion.transform.position);
            yield return null;

        }
        if (Vector3.Distance(NPC1.gameObject.transform.position, NPCDadStartPosiotion.transform.position) <= 2.5)
        {
            nav.isStopped = true;
            nav.ResetPath();

            NPC1.gameObject.transform.position = behavior.HomePoint.position;
            NPC1.gameObject.SetActive(false);
        }
        
    }

    IEnumerator turtorialStart()
    {
        yield return new WaitForSeconds(4);
