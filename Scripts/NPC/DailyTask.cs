...
public class DailyTask : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        interact=  GetComponent<PBRInteract>(); 
    }
    ...
    {
        DailyRestTask();
        Debug.Log(TimeManager.Instance.timestamp.hour);
    }
                ...
                if (TimeManager.Instance.timestamp.hour == taskList[i].time_.hour && TimeManager.Instance.timestamp.minute == taskList[i].time_.minute && !IsTaskDone(i) ) // co the them 1 Hanh dong khac cho ngay dac biet - chua nghi nen lam gi
                    {
                       if(!interact.isOnWorkingTimes)
                       {
                        ...
                        {
                            behavior.SwitchStateAction(NPCAction.DoNothing);
                        }
                        else if (taskList[i].TaskGoal == "MeetEmma")
                        {

                            behavior.SwitchStateAction(NPCAction.MeetEmma);
                        }
                        else if (taskList[i].TaskGoal == "GoingSomeWhere")
                        {

                            behavior.SwitchStateAction(NPCAction.GoingSomeWhere);
                        }
                        else if (taskList[i].TaskGoal == "Dancing")
                        {

                            behavior.SwitchStateAction(NPCAction.Dancing);
                        }
                        else if (taskList[i].TaskGoal == "Thinking")
                        {

                            behavior.SwitchStateAction(NPCAction.Thinking);
                        }
                        else if (taskList[i].TaskGoal == "MeetSeller")
                        {

                            behavior.SwitchStateAction(NPCAction.MeetSeller);
                        }
                        else if (taskList[i].TaskGoal == "MeetNPC")
                        {
                            behavior.NPCGonnaMeet = taskList[i].NPC;
                            behavior.SwitchStateAction(NPCAction.MeetNPC);
                        }
